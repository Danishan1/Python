try:
    ...
    sys.path.insert(
        0, os.path.expanduser("~") + "/Virtual-Voice-Assistant"
    )  # adding voice assistant directory to system path
...
def main(query):
    ...
    if (
        ("google" in query and "search" in query)
        or ("google" in query and "how to" in query)
        or "google" in query
    ):
        googleSearch(query)
        return
    elif (
        ("youtube" in query and "search" in query)
        or "play" in query
        or ("how to" in query and "youtube" in query)
    ):
        youtube(query)
        return
    ...
    elif intent == "system_info" and (
        "info" in query or "specs" in query or "information" in query
    ):
        info = systemInfo()
        speak(info)
        done = True
    ...
    elif intent == "exit" and (
        "exit" in query or "terminate" in query or "quit" in query
    ):
        exit(0)
    if not done:
        answer = get_general_response(query)
