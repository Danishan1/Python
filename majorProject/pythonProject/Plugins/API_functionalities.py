...
def get_news():
    try:
        ...
        for i in range(10):
            top_news += (
                re.sub(
                    r"[|-] [A-Za-z0-9 |:.]*", "", top_headlines["articles"][i]["title"]
                ).replace("’", "'")
                + "\n"
            )
        return top_news
    ...
    try:
        if city:
            response = requests.get(
                f"https://api.openweathermap.org/data/2.5/weather?q={city}&appid={OPENWEATHERMAP}&units=metric"
            ).json()
        else:
            response = requests.get(
                f'https://api.openweathermap.org/data/2.5/weather?q={get_ip(True)["city"]}&appid={OPENWEATHERMAP}&units=metric'
            ).json()
        weather = (
            f'It\'s {response["main"]["temp"]}° Celsius and {response["weather"][0]["main"]}\n'
...
def get_popular_movies():
    try:
        response = requests.get(
            f"https://api.themoviedb.org/3/movie/popular?api_key={TMDB}&region=IN&sort_by=popularity.desc&"
...
def get_popular_tvseries():
    try:
        response = requests.get(
            f"https://api.themoviedb.org/3/tv/popular?api_key={TMDB}&region=IN&sort_by=popularity.desc&"
            f"primary_release_year={datetime.date.today().year}"
        ).json()
    except requests.exceptions.RequestException:
        return None
